...
def ex3(length, width):
    ...
    ### BEGIN SOLUTION
    area = length*width
    return (f"The area of a rectangle with length {length} and width {width} is {area:.2f}.")
...
def ex6(weight, height):
    ...
    ### BEGIN SOLUTION
    return round((weight * 720 /(height*height)),2)
    ### END SOLUTION
...
def ex7(no_one_liter_bottles, no_more_than_one_liter_bottles):
    ...
    ### BEGIN SOLUTION
    amount = (no_more_than_one_liter_bottles * 0.25) + (no_one_liter_bottles*0.1)
...
def ex8(a, b):
    ...
    f"The quotient when {a} is divided by {b} is {int(a/b)}.\n" +
...
def ex9(P, r, t):
    ...
    ### BEGIN SOLUTION
    return  (f'After {t} years at {r}%, the investment will be worth ${P* (1+((r*t)/100)):.2f}.')
    ### END SOLUTION
